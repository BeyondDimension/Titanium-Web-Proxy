...
internal struct ByteString : IEquatable<ByteString>
{
    ...
    public static explicit operator ByteString(string str) => new(Encoding.ASCII.GetBytes(str));

    public static implicit operator ByteString(byte[] data) => new(data);

    public static implicit operator ByteString(ReadOnlyMemory<byte> data) => new(data);
