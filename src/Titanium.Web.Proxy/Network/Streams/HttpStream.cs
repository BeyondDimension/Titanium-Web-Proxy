...
internal class HttpStream : Stream, IHttpStreamWriter, IHttpStreamReader, IPeekStream
{
    ...
    public override int Read(byte[] buffer, int offset, int count)
    {
        ...
        var available = Math.Min(bufferLength, count);
        if (available > 0)
        {
...
#endif
    {
        ...
        var available = Math.Min(bufferLength, buffer.Length);
        if (available > 0)
        {
    ...
    public bool FillBuffer()
    {
        ...
        try
        {
            var readBytes = baseStream.Read(streamBuffer, bufferLength, streamBuffer.Length - bufferLength);
            result = readBytes > 0;
            if (result)
            {
    ...
    public async ValueTask<bool> FillBufferAsync(CancellationToken cancellationToken = default)
    {
        ...
        var bytesToRead = streamBuffer.Length - bufferLength;
        if (bytesToRead == 0)
        {
            return false;
        }
    ...
    public async Task CopyBodyAsync(RequestResponseBase requestResponse, bool useOriginalHeaderValues, IHttpStreamWriter writer, TransformationMode transformation, bool isRequest, SessionEventArgs args, CancellationToken cancellationToken)
    {
        ...
        var contentLength = useOriginalHeaderValues ? requestResponse.OriginalContentLength : requestResponse.ContentLength;
        ...
        var contentEncoding = useOriginalHeaderValues ? requestResponse.OriginalContentEncoding : requestResponse.ContentEncoding;
